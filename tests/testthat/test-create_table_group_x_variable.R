test_that("results are showned correclty", {
  expected_results <- readRDS(testthat::test_path("fixtures/group_x_variable", "table_group_x_variable_3stats.RDS"))
...
test_that("if some arguments are missing, it throws an error.", {
  expect_error(
    create_table_group_x_variable(presentresults_resultstable,
